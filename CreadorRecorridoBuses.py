...
from services import DataManager, Simulador
from gui import Styles

class CreadorRecorridosBuses:
    def __init__(self, root):
        self.root = root
        self.root.title("🚍 BuScanGo - Sistema de Gestión de Buses")
    ...
    def crear_pestana_buses(self, notebook):
        ...
        self.combo_rutas = ttk.Combobox(form_frame, width=30)
        self.combo_rutas.grid(row=1, column=1, pady=5, padx=5)
        
        ttk.Button(form_frame, text="🚌 Agregar Bus", 
                  command=self.agregar_bus).grid(row=2, column=0, columnspan=2, pady=10)
        
        # Lista de buses
        list_frame = ttk.LabelFrame(frame_buses, text=" Buses Registrados ", padding="10")
        list_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.tree_buses = ttk.Treeview(list_frame, 
                                  columns=('Numero', 'Ruta', 'Estado'), 
                                  show='headings')
        self.tree_buses.heading('Numero', text='N° Bus')
    ...
    def crear_pestana_simulacion(self, notebook):
        ...
        notebook.add(frame_simulacion, text="▶️ Simulación")
        
        # Controles de simulación
        control_frame = ttk.Frame(frame_simulacion)
        control_frame.pack(pady=10)
        
        ttk.Button(control_frame, text="▶️ Iniciar", 
                  command=self.iniciar_simulacion).pack(side='left', padx=5)
        ttk.Button(control_frame, text="⏸️ Pausar", 
        ...
        # Área de visualización
        self.text_simulacion = tk.Text(frame_simulacion, height=20, width=60)
        self.text_simulacion.pack(pady=10, padx=10)
    ...
    def actualizar_combo_rutas(self):
        """Actualiza el combobox de rutas en la pestaña de gestión de buses"""
        rutas = list(self.rutas.keys())
        self.combo_rutas['values'] = rutas
        if rutas:
            self.combo_rutas.set(rutas[0])

    def actualizar_combo_rutas_ver(self):
        """Actualiza el combobox de rutas en la pestaña de visualización"""
        rutas = list(self.rutas.keys())
        self.combo_rutas_ver['values'] = rutas
        if rutas:
            self.combo_rutas_ver.set(rutas[0])

    def agregar_bus(self):
        numero_bus = self.entry_numero_bus.get().strip()
        ruta = self.combo_rutas.get()
    ...
    def mostrar_recorrido(self, event=None):
        """Muestra el detalle de la ruta seleccionada en el área de texto"""
        ruta_seleccionada = self.combo_rutas_ver.get()
        if ruta_seleccionada in self.rutas:
            ruta = self.rutas[ruta_seleccionada]
            self.text_info.delete(1.0, tk.END)
            self.text_info.insert(tk.END, f"Ruta: {ruta.nombre}\n\n")
            for i, parada in enumerate(ruta.paradas, 1):
                self.text_info.insert(tk.END, f"{i}. {parada}\n")
